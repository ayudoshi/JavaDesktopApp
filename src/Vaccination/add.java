...
public class add implements ActionListener{
    ...
    JTextField t1,t2,t4,t6,t7,t10;
    JButton b1,b;
    String name;

    add(String n){
        name=n;
        f = new JFrame("Member Details");
...
//        id15.add(lab1);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource() == b1 && m==0) {
            try {

                conn cc = new conn();

                String q1 = "select * from data order by id desc limit 1";
                ResultSet rs1 = cc.s.executeQuery(q1);
                rs1.next();
                int id = rs1.getInt("id");
                i=(id+1);

                String q = "insert into data values('" + a + "','" + bb + "','" + d + "','" + ff + "','" + g + "','" + j + "','"+ i +"','"+name+"')";
                cc.s.executeUpdate(q);
                JOptionPane.showMessageDialog(null, "Details Successfully Inserted");
                JOptionPane.showMessageDialog(null, "Your Id is:" + i + "Please remember it. It is very important");
                f.setVisible(false);
                new details();
            } catch (Exception ee) {
